...
export default {
  ...
  data(){
    return{
      username:'',
      password:'',
      validataresult:{},  //用于接收滑动验证返回的数据，用于服务端SDK的二次验证
    }
  },
  methods:{
    loginhandler(){       //进行服务端 SDK 进行二次验证
      console.log('进行登录！');
      let params={      //需要传入五个参数
        username:this.username,
        password:this.password,
        geetest_challenge:this.validataresult.geetest_challenge,
        geetest_validate:this.validataresult.geetest_validate,
        geetest_seccode:this.validataresult.geetest_seccode};
      this.$http.userlogin(params)
        .then(res=>{
          if(res.error_no===0){
            console.log('登录成功，登录返回的信息',res);
            this.$router.push({
                name:'home'   //登录成功，跳转到首页
            });
            //存储信息
      ...
      this.$http.geetest()
        .then(res=>{
          ...
          initGeetest({
              ...
              captchaObj.onReady(()=>{    //准备加载
              }).onSuccess(()=>{          //加载成功
                let res=captchaObj.getValidate();//取得滑动验证返回的数据
                this.validataresult=res;
                console.log('滑动验证',this.validataresult);
              }).onError(()=>{            //加载出错
                //your code
              })
